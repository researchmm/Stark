...
import time
import cv2 as cv
# from lib.test.utils.visdom import Visdom
import matplotlib.pyplot as plt
import matplotlib.patches as patches
from lib.test.utils.plotting import draw_figure, overlay_mask
from lib.test.utils import convert_vot_anno_to_rect
...
import torch
# from lib.toolkit.utils.region import vot_overlap, vot_float2str
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        """2021.1.15 Add VOT support"""
        if 'vot' in seq.dataset:
            return self._track_sequence_vot(tracker, seq)
        ...
        for key in ['target_bbox', 'segmentation', 'all_boxes', 'all_scores']:
            if key in output and len(output[key]) <= 1:
                output.pop(key)

        return output

    def _track_sequence_vot(self, tracker, seq):
        ...
        for frame_num, frame_path in enumerate(seq.frames):
            if frame_num == frame_counter:
                # Initialize
                image = self._read_image(seq.frames[frame_num])

                start_time = time.time()
                init_info = {"init_bbox": list(seq.ground_truth_rect[frame_num])}
                out = tracker.initialize(image, init_info)
                if out is None:
                    out = {}
                ...
                out = tracker.track(image, info)
                '''compute overlap and determine whether to reset'''
                target_bbox = out["target_bbox"]
                gt_bbox = list(seq.ground_truth_rect[frame_num])
                overlap = vot_overlap(target_bbox, gt_bbox, (image.shape[1], image.shape[0]))
                if overlap > 0:
                    prev_output = OrderedDict(out)
                    _store_outputs(out)
                elif frame_num > frame_counter:
                    _store_outputs({'target_bbox': 2})
                    frame_counter = frame_num + 5
                    lost_number += 1
            else:
                _store_outputs({'target_bbox': 0})

        for key in ['target_bbox', 'all_boxes']:
            if key in output and len(output[key]) <= 1:
                output.pop(key)
