...
class STARKSPLUSSPActor(BaseActor):
    ...
    def forward_pass(self, data, run_box_head, run_cls_head):
        ...
        feat_dict_list.append(self.net(img=NestedTensor(search_img, search_att), mode='backbone', zx="search"))
    ...
    #     pos_m_tl, pos_m_br = ptl_label > 1/400, pbr_label > 1/400  # positive mask
    #     neg_m_tl, neg_m_br = ~pos_m_tl, ~pos_m_br  # negative mask
    #     ptl_label_pos, ptl_label_neg = ptl_label[pos_m_tl], ptl_label[neg_m_tl]
    #     pbr_label_pos, pbr_label_neg = pbr_label[pos_m_br], pbr_label[neg_m_br]
    #     n_layer = len(ptl_list) - 1
    #     for i in range(n_layer):
    #         prob_tl, prob_br = ptl_list[i], pbr_list[i]
    #         ptl_pos, ptl_neg = prob_tl[pos_m_tl], prob_tl[neg_m_tl]
    #         pbr_pos, pbr_neg = prob_br[pos_m_br], prob_br[neg_m_br]
    def compute_distill_losses(self, output_dict):
        ptl_list, pbr_list = output_dict["prob_tl"], output_dict["prob_br"]
        ptl_label, pbr_label = ptl_list[-1], pbr_list[-1]
        n_layer = len(ptl_list) - 1
